...
const BEST_RESULTS = document.getElementById('bestResults');
...
const drawMatrix = () => {
    ...
    if (timeCounter) {
        ...
        if (gameFinal) {
            ...
            FIELD.innerHTML = `<div class="puzzle puzzle1x1 fontSize32">Ура! Вы решили головоломку за: <br> ${minutes}:${seconds} и ${createScore} ходов</div>`;
...
CONTROL_ELEMENTS.addEventListener('click', (event) => {
    ...
    if (id === 'newGame') {
        ...
        RESULTS.innerHTML ='';
        let bestResultsArr = localStorage[result].split('-').filter(el => el.length>0);
        RESULTS.innerHTML ='';
        for (let i=0; i<bestResultsArr.length; i++){
            RESULTS.innerHTML += `<li class="point">${bestResultsArr[i]}</li>`;
        }
        BEST_RESULTS.classList.add('bestResultsActive')
        timeCounter = false;
    } else if (id === 'pause') {
        if (timeCounter) {
            timeCounter = false;
...
BEST_RESULTS.addEventListener('click', (event) => {
    let id = '';
    id = event.target.id === '' ? event.target.parentNode.id : event.target.id;
    if (id === 'closeBestResultsBlock') {
        BEST_RESULTS.classList.remove('bestResultsActive');
    }
});
